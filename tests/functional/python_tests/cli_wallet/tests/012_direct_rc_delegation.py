...
if __name__ == "__main__":
    with Test(__file__):
        with CliWallet( args ) as wallet:
            ...
            delegation_to_from = wallet.list_rc_direct_delegations([receiver, delegator], 1000, "by_to_from")['result'][0]

            assert(delegation_from_to['from'] == delegator)
            assert(delegation_from_to['to'] == receiver)
            assert(delegation_from_to['delegated_rc'] == 10)

            assert(delegation_to_from['from'] == delegator)
            assert(delegation_to_from['to'] == receiver)
            assert(delegation_to_from['delegated_rc'] == 10)
